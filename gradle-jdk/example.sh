...
oc new-build jorgemoralespou/s2i-java~https://github.com/jorgemoralespou/s2i-java --context-dir=/test/test-app-gradle/ --name=builder
...
oc new-build --name=runtime --docker-image=registry.access.redhat.com/redhat-openjdk-18/openjdk18-openshift \
...
oc new-app runtime --name=test-app-gradle
oc expose svc/test-app-gradle

# Wait for the rollout TODO: There's no liveness and rediness'

# Print the endpoint URL
echo “Access the service at http://$(oc get route/test-app-gradle -o jsonpath='{.status.ingress[0].host}')/” 
